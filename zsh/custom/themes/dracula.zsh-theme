...
PROMPT+='%(1V:%F{yellow}:%(?:%F{magenta}:%F{red}))%B$(dracula_arrow start)'
# }}}
...
PROMPT+='%F{magenta}%B$(dracula_time_segment)'
# }}}
...
ZSH_THEME_GIT_PROMPT_CLEAN=") %F{magenta}%B✔ "
...
# Linebreak {{{
PROMPT+='%(1V:%F{yellow}:%(?:%F{magenta}:%F{red}))%B$(dracula_arrow end)'
# }}}
